...
def train(epochs, batch_size,output_dir, model_save_dir):
    ...
    for e in range(1, epochs+1):
        ...
        for sp in range(0,batch_count,1):
            ...
            Y_batch  =  np.expand_dims(Y_batch,axis=img_dims)
            edge_y = np.expand_dims(edge_y,axis=img_dims)
            edge_x = np.expand_dims(edge_x,axis=img_dims)
            '''for i in range(0,batch_size):
                edge_batch.append(cv2.Canny(np.asarray(np.uint8(Y_batch[i])),10,100))
            edge_batch = np.asarray(edge_batch)
            edge_batch  =  np.expand_dims(edge_batch,axis=img_dims)

            for i in range(0,batch_size):
                input_edge.append(cv2.Canny(np.asarray(np.uint8(X_batch[i])),10,100))
            input_edge = np.asarray(input_edge) 
            input_edge =np.expand_dims(input_edge,axis=img_dims)'''
            #print(Y_batch.shape)
...
edge_x_test  =  np.expand_dims(edge_x_test,axis=img_dims)
...
Y_test  =  np.expand_dims(Y_test,axis=img_dims)
